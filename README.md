...
- "Rexplorer" preempts problems related to Start menu/app windows/taskbar by killing all processes called "explorer.exe" and restart them with restoring all instances of Windows-Explorer / system control. Until recently (year 2020), there was no app that could do that. Another problem, we can get around: As you may know, when copying files or folders the progress window is also part of explorer.exe. In killing it, on all OS before Windows 8 it interrupts also the copying. How lucky we must be, start menu freezing only happends on Windows 10, so Rexplorer don't harm any open copy process (it only closes the progress window).
