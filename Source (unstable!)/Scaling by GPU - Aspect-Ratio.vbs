' ***************************************************************************
' 
' This script scales the screen with the graphics card (GPU)
' while maintaining the aspect ratio
' Dieses Skript lässt den Bildschirm mit der Grafikkarte (GPU)
' unter Beihaltung des Seitenverhältnisses skalieren
'
' Author: Dragodraki
' Last updated: 2024
...
If Not (isnull(aSubKeys)) Then
    For Each sSubkey In aSubKeys
		...
		BinaryReplacement = 5		'05 in hexadezimal ist das Ziel! (Angabe muss erfolgen: ohne Anführungsstriche = dezimal!)
		...
		BinaryReplacement = 241		'f1 in hexadezimal ist das Ziel! (Angabe muss erfolgen: ohne Anführungsstriche = dezimal!)
	    	oReg.GetBinaryValue cHKLM, path, "ScalingConfig", bArray
	   	 'msgbox bArray(BinaryPositionNumberToChange - 1)	' Erste Zahl ist auszulesene Binary-Position und zweite korrigiert einen Darstellungsfehler
	    	If not bArray(BinaryPositionNumberToChange - 1) = BinaryReplacement AND not NvidiaTest_1 = "NeueMethodeDwordKorrekt" AND not NvidiaTest_2 = "Aus" Then
		'Ändere eine bestimmte Position der Binary-Strings
    ...
    For Each sSubkey In aSubKeys
        sTmpKeyName1 = sKeyPath & "\" & sSubKey
	   	...
	   	' Lese zuerst den Binary-String aus und schaue, ob Änderung notwendig ist
	    	...
	    	path = sTmpKeyName1
		BinaryPositionNumberToChange = 9
		BinaryReplacement = 133		'85 in hexadezimal ist das Ziel! (Angabe muss erfolgen: ohne Anführungsstriche = dezimal!)
	    	oReg.GetDwordValue cHKLM, path, "ScalingConfig", bArray	' Neue Methode von Nvidia (Dword-Wert: 0=Anzeige, 1=GPU)
		if bArray = 1 then
		NvidiaTest_1 = "NeueMethodeDwordKorrekt"
		End if
		...
		Return = oReg.SetBinaryValue(cHKLM, path, "ScalingConfig", bArray)
	   	...
	   	' Lese zuerst den Binary-String aus und schaue, ob Änderung notwendig ist
	    	...
	    	path = sTmpKeyName1
		BinaryPositionNumberToChange = 13
		BinaryReplacement = 113		'71 in hexadezimal ist das Ziel! (Angabe muss erfolgen: ohne Anführungsstriche = dezimal!)
	    	oReg.GetBinaryValue cHKLM, path, "ScalingConfig", bArray
	   	'msgbox bArray(BinaryPositionNumberToChange - 1)	' Erste Zahl ist auszulesene Binary-Position und zweite korrigiert einen Darstellungsfehler
	    	If not bArray(BinaryPositionNumberToChange - 1) = BinaryReplacement AND not NvidiaTest_1 = "NeueMethodeDwordKorrekt" AND not NvidiaTest_2 = "Aus" Then
		'Ändere eine bestimmte Position der Binary-Strings
