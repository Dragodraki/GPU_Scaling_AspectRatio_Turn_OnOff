' ***************************************************************************
' 
' This script scales the screen to stretched full screen
' using the graphics card (GPU)
' Dieses Skript lässt den Bildschirm mit der Grafikkarte (GPU)
' zum gestreckten Vollbild skalieren
'
' Author: Dragodraki
' Last updated: 2024
...
If Not (isnull(aSubKeys)) Then
	...
	'msgbox sTmpKeyName2
	    ...
	    ' Lese zuerst den Binary-String aus und schaue, ob Änderung notwendig ist
...
If Not (isnull(aSubKeys)) Then
    For Each sSubkey In aSubKeys
		...
		BinaryReplacement = 3		'03 in hexadezimal ist das Ziel! (Angabe muss erfolgen: ohne Anführungsstriche = dezimal!)
		...
		BinaryReplacement = 239		'ef in hexadezimal ist das Ziel! (Angabe muss erfolgen: ohne Anführungsstriche = dezimal!)
	    	oReg.GetBinaryValue cHKLM, path, "ScalingConfig", bArray
	   	 'msgbox bArray(BinaryPositionNumberToChange - 1)	' Erste Zahl ist auszulesene Binary-Position und zweite korrigiert einen Darstellungsfehler
	    	If not bArray(BinaryPositionNumberToChange - 1) = BinaryReplacement AND not NvidiaTest_1 = "NeueMethodeDwordKorrekt" AND not NvidiaTest_2 = "Aus" Then
		'Ändere eine bestimmte Position der Binary-Strings
    ...
    For Each sSubkey In aSubKeys
        sTmpKeyName1 = sKeyPath & "\" & sSubKey
	   	...
	   	' Lese zuerst den Binary-String aus und schaue, ob Änderung notwendig ist
	    	...
	    	path = sTmpKeyName1
		BinaryPositionNumberToChange = 9
		BinaryReplacement = 131		'83 in hexadezimal ist das Ziel! (Angabe muss erfolgen: ohne Anführungsstriche = dezimal!)
	    	oReg.GetDwordValue cHKLM, path, "ScalingConfig", bArray	' Neue Methode von Nvidia (Dword-Wert: 0=Anzeige, 1=GPU)
		if bArray = 1 then
		NvidiaTest_1 = "NeueMethodeDwordKorrekt"
		End if
		...
		Return = oReg.SetBinaryValue(cHKLM, path, "ScalingConfig", bArray)
	   	...
	   	' Lese zuerst den Binary-String aus und schaue, ob Änderung notwendig ist
	    	...
	    	path = sTmpKeyName1
		BinaryPositionNumberToChange = 13
		BinaryReplacement = 111		'6f in hexadezimal ist das Ziel! (Angabe muss erfolgen: ohne Anführungsstriche = dezimal!)
	    	oReg.GetBinaryValue cHKLM, path, "ScalingConfig", bArray
	   	'msgbox bArray(BinaryPositionNumberToChange - 1)	' Erste Zahl ist auszulesene Binary-Position und zweite korrigiert einen Darstellungsfehler
	    	If not bArray(BinaryPositionNumberToChange - 1) = BinaryReplacement AND not NvidiaTest_1 = "NeueMethodeDwordKorrekt" AND not NvidiaTest_2 = "Aus" Then
		'Ändere eine bestimmte Position der Binary-Strings
...
' --- AMD: Ggf. Laufende Instanz für später merken
Counter = 0
strComputer = "."
Set objWMIService = GetObject("winmgmts:" & "{impersonationLevel=impersonate}!\\" & strComputer & "\root\cimv2")
...
' --- NVIDIA: Ggf. Laufende Instanz für später merken
...
NVIDIAPath=left(objprocess.ExecutablePath, instr(objprocess.ExecutablePath, search)-0)
    ' Beende NVIDIA, da ansonsten das Programmoberfläche die Änderungen nicht anzeigt
    ' msgbox "Prozesspfad: " & objProcess.ExecutablePath
    nPos = InStr(objProcess.ExecutablePath, ProcessPfad)  'binary comparison
    if nPos="0" Then
    ' msgbox "Debugginginfo: String nicht gefunden, nicht löschen..."
    Else
    ' msgbox "Debugginginfo: String ist Teil des ausgeführten Pfads - Prozess wird gekillt..."
...
' --- Kille StikyNot.exe, wenn Pfad und Prozessname stimmt (und Kurznotizen überhaupt laufen)
...
' Hilfe -Apptitle- : Fensterüberschrift (komplett oder Teil davon), z.B. "Brennvorgang Free 0% abges" [...]
...
Sleep "6000"	'Sleep-Modus exklusiv für "PScript.exe" (= Portable VBS)
WScript.Sleep "6000"	'Sleep-Modus für windowseigenen Script-Host "wscript.exe" und "cscript.exe"
...
' Verwalte RadeonSoftware.exe im Hintegergrund - dafür braucht es cncmd.exe
Set objShell = CreateObject("WScript.Shell")
